...
Usage :
  ...
  (identifiants Supabase MPB_EMAIL/MPB_PASSWORD également requis, voir
  mpb_auth.py — RLS oblige depuis le verrouillage de la base.)

Ne JAMAIS committer d'email/mot de passe en dur dans ce fichier — toujours
via variables d'environnement (ou un fichier .env local, voir .env.example
et mpb_auth.py).

Le jeton de session Zepp (apptoken) est mis en cache localement
(.zepp_token_cache.json, non suivi par git) et réutilisé tant qu'il est
accepté par l'API : se reconnecter avec le mot de passe complet à chaque
exécution ressemblerait à un comportement de bot pour les systèmes anti-abus
de Zepp, avec un vrai risque de blocage du compte. Le mot de passe n'est
donc utilisé que la première fois, et à nouveau seulement si le jeton en
cache est rejeté (session expirée côté Zepp).
...
# Jeton d'accès Supabase obtenu au démarrage (voir __main__) — la clé anon
# seule n'a plus accès aux données depuis le verrouillage des policies RLS.
ACCESS_TOKEN = None

LOCAL_TZ = ZoneInfo("Europe/Paris")

# Cache local du jeton de session Zepp (apptoken) — voir docstring en tête de
# fichier pour pourquoi on évite de se reconnecter avec le mot de passe à
# chaque exécution.
_TOKEN_CACHE_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), ".zepp_token_cache.json")
# Large marge sous une éventuelle expiration côté Zepp (inconnue, jamais
# documentée) — de toute façon, une réponse 401/403 déclenche une
# reconnexion immédiate quelle que soit l'ancienneté du cache.
_TOKEN_CACHE_MAX_AGE_S = 20 * 24 * 3600
...
def _load_cached_session():
    if not os.path.isfile(_TOKEN_CACHE_FILE):
        return None
    try:
        with open(_TOKEN_CACHE_FILE) as f:
            cache = json.load(f)
        if time.time() - cache["obtained_at"] > _TOKEN_CACHE_MAX_AGE_S:
            return None
        return cache["app_token"], cache["user_id"], cache["host"]
    except (OSError, ValueError, KeyError):
        return None


def _store_cached_session(app_token, user_id, host):
    with open(_TOKEN_CACHE_FILE, "w") as f:
        json.dump({"app_token": app_token, "user_id": user_id, "host": host, "obtained_at": time.time()}, f)
...
def get_session(email: str, password: str, force_login: bool = False) -> tuple[str, str, str]:
    """Session Zepp valide — réutilise le jeton en cache tant qu'il existe,
    ne se reconnecte avec le mot de passe complet que si force_login=True
    (cache absent/expiré, ou rejeté par l'API — voir run_sync)."""
    if not force_login:
        cached = _load_cached_session()
        if cached:
            return cached
    session = zepp_login(email, password)
    _store_cached_session(*session)
    return session
...
def collect_all(host: str, app_token: str, user_id: str, days: int) -> list:
    today = date.today()
...
def main():
    ...
    mpb_auth._load_env_file()  # même fichier .env que MPB_EMAIL/MPB_PASSWORD, voir .env.example
    email = os.environ.get("ZEPP_EMAIL")
    password = os.environ.get("ZEPP_PASSWORD")
    if not email or not password:
        sys.exit("ZEPP_EMAIL et ZEPP_PASSWORD doivent être définis en variables d'environnement (ou dans .env).")

    app_token, user_id, host = get_session(email, password)
    print(f"Session Zepp (user_id={user_id}, host={host})")

    try:
        rows = collect_all(host, app_token, user_id, args.days)
    except requests.HTTPError as e:
        status = e.response.status_code if e.response is not None else None
        if status not in (401, 403):
            raise
        print("Jeton en cache rejeté par Zepp — reconnexion avec le mot de passe…")
        _clear_cached_session()
        app_token, user_id, host = get_session(email, password, force_login=True)
        rows = collect_all(host, app_token, user_id, args.days)
